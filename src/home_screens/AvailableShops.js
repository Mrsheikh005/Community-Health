...
const AvailableShops = () => {
	...
	useEffect(() =>{
		fetch('https://pharmacy.shahjahanxd.xyz/api/shop/get-all-shops')
				.then((response) => response.json())
				.then((json) => {
					//   return json.movies;
					setData(json.data);
	...
	return (
	  <>
		<SafeAreaView style={{ flex: 1, justifyContent: 'space-between', marginBottom:'20%'}}>
				<TransparentHeader title="Order Pharmacy" />
								...
								>
									<View style={{ flexDirection: 'row', flex: 1 }}>
										<View style={{ alignItems: 'center', flex: 1 }}>
											<Image style={{ width: '100%', height: 120 }} source={{uri:'https://pharmacy.shahjahanxd.xyz/images/' + item.shopProfile} }/>
										</View>
									</View>
									<View
										style={{
